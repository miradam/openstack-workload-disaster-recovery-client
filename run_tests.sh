...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run dragonclient's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./dragonclient/tests"
    fi
  ...
  if [ setup.cfg -nt dragonclient.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='dragonclient/*' --omit='dragonclient/openstack/common/*' -d covhtml -i
  fi
